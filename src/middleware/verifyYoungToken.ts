...
export default async (req, _, next) => {
  try {
    ...
    const tokenAgeInMinutes =
      (utcSecondsSinceEpoch - Date.parse(refreshTokenFamily.createdAt) / 1000) / 60;
    if (tokenAgeInMinutes > 20) {
      refreshTokenFamily.deleteOne();
      throw {
        token: "Трябва да потвърдим Вашата самоличност. Излезте и влезте обратно в профила си",
        status: 401,
      };
    }
    next();
